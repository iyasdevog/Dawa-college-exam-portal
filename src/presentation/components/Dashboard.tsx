...
const Dashboard: React.FC<DashboardProps> = ({ onNavigateToManagement }) => {
    ...
    // Enhanced mobile-optimized export functionality - Using native print for real PDF generation
    const handleMobileExportPDF = async () => {
        ...
        setTouchFeedback({
            isVisible: true,
            action: 'exporting',
            message: 'Preparing official PDF report...'
        });

        // Small delay to show feedback and let UI settle
        setTimeout(() => {
            window.print();
            setTouchFeedback(prev => ({ ...prev, isVisible: false }));
        }, 800);
