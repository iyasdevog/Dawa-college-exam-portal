    ...
    >
      <div className="flex items-center justify-between h-full">
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          {/* Page Title - Mobile Optimized with proper touch targets */}
          <h1
            className={`
            ...
            role="heading"
            aria-level={1}
            aria-label={`Current page: ${getViewTitle(activeView)}`}
          >
            {getViewTitle(activeView)}
          </h1>

          {/* Cloud Status - Mobile Priority with enhanced accessibility */}
          <div className={`${isMobile ? 'ml-auto' : 'ml-4'} touch-target-min`}>
        ...
        {!isMobile && (
          <div className="flex items-center gap-4">
            ...
            {/* Connection Status Icon with enhanced touch targets */}
            <button
              className={`
              ...
              `}
              aria-label={`Connection status: ${isCloudActive ? 'Online' : 'Offline'}`}
              role="button"
              tabIndex={0}
            >
              <i className={`
      ...
      {/* Mobile-specific secondary info bar with safe area support */}
      {isMobile && (
        <div
          className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-emerald-500 to-blue-500 opacity-20"
          aria-hidden="true"
        ></div>
      )}
    </header>
