...
async function auditAll8ClassesDeep() {
    console.log('\n=== DEEP AUDIT OF ALL 8 CLASSES IN ClassResults LOGIC (2025-2026-Odd) ===\n');
    ...
    for (const selectedClass of classes) {
        ...
        console.log(`Students count: ${classStudents.length}`);
        ...
        console.log(`Rendered Subject Columns (${classSubjects.length}):`);
        ...
        classSubjects.forEach(s => {
            ...
            classStudents.forEach(cs => {
                const termData = cs.academicHistory[activeTerm];
                const m = getMarkForSubject(termData.marks, s, termData.subjectMetadata);
                if (m && !m.isSupplementary && (m.total > 0 || m.int !== undefined || m.ext !== undefined)) filledCount++;
            });

            console.log(`  - [${s.id}] "${s.name}" (type: ${s.subjectType}) → Filled: ${filledCount}/${classStudents.length}`);
        });
    }
}

auditAll8ClassesDeep().then(() => process.exit(0)).catch(err => { console.error(err); process.exit(1); });
