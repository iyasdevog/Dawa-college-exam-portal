...
import { StudentRecord, SubjectConfig, SupplementaryExam, SubjectMarks } from '../types';
...
export class DataService {
    ...
    async getAllStudents(): Promise<StudentRecord[]> {
        ...
        try {
            ...
            const querySnapshot = await getDocs(collection(db, this.studentsCollection));
    ...
    async getStudentByAdNo(adNo: string): Promise<StudentRecord | null> {
        ...
        try {
            const q = query(
                collection(db, this.studentsCollection),
                where('adNo', '==', adNo)
            );
    ...
    async addStudent(student: Omit<StudentRecord, 'id'>): Promise<string> {
        ...
        try {
            const docRef = await addDoc(collection(db, this.studentsCollection), student);
    ...
    async updateStudent(id: string, updates: Partial<StudentRecord>): Promise<void> {
        ...
        try {
            const docRef = doc(db, this.studentsCollection, id);
    ...
    async addSubject(subject: Omit<SubjectConfig, 'id'>): Promise<string> {
        ...
        try {
            const docRef = await addDoc(collection(db, this.subjectsCollection), subject);
    ...
    async updateSubject(id: string, updates: Partial<SubjectConfig>): Promise<void> {
        ...
        try {
            const docRef = doc(db, this.subjectsCollection, id);
            await updateDoc(docRef, updates);
    ...
    async enrollStudentInSubject(subjectId: string, studentId: string): Promise<void> {
        ...
        try {
            ...
            if (!enrolledStudents.includes(studentId)) {
                ...
                await updateDoc(doc(db, this.subjectsCollection, subjectId), {
                    enrolledStudents
                });
            }
    ...
    async addSupplementaryExam(supplementaryExam: Omit<SupplementaryExam, 'id'>): Promise<string> {
        ...
        try {
            const docRef = await addDoc(collection(db, this.supplementaryExamsCollection), supplementaryExam);
    ...
    async getSupplementaryExamsByStudent(studentId: string): Promise<SupplementaryExam[]> {
        ...
        try {
            const q = query(
                collection(db, this.supplementaryExamsCollection),
    ...
    async updateSupplementaryExamMarks(supplementaryExamId: string, marks: SubjectMarks): Promise<void> {
        ...
        try {
            const docRef = doc(db, this.supplementaryExamsCollection, supplementaryExamId);
    ...
    async getStudentsWithSupplementaryExams(subjectId: string, year: number): Promise<{ student: StudentRecord, supplementaryExam: SupplementaryExam }[]> {
        ...
        try {
            ...
            for (const suppExam of supplementaryExams) {
                const studentDoc = await getDoc(doc(db, this.studentsCollection, suppExam.studentId));
    ...
    subscribeToSubjects(callback: (subjects: SubjectConfig[]) => void): Unsubscribe {
        ...
        return onSnapshot(
            collection(db, this.subjectsCollection),
            (snapshot) => {
            ...
            for (let i = 0; i < students.length; i += batchSize) {
                ...
                for (const student of currentBatch) {
                    try {
                        ...
                        const docRef = doc(collection(db, this.studentsCollection));
                        batch.set(docRef, {
                            ...student,
                            marks: student.marks || {},
    ...
    async calculateClassRankings(className: string): Promise<void> {
        ...
        try {
            ...
            sortedStudents.forEach((student, index) => {
                const docRef = doc(db, this.studentsCollection, student.id);
    ...
    async testConnection(): Promise<boolean> {
        ...
        try {
            ...
            const testQuery = await getDocs(collection(db, this.studentsCollection));
    ...
    async clearAllData(): Promise<void> {
        ...
        try {
            ...
            // Get all students
            const studentsSnapshot = await getDocs(collection(db, this.studentsCollection));
    ...
    async clearAllSubjects(): Promise<void> {
        ...
        try {
            ...
            // Get all subjects
            const subjectsSnapshot = await getDocs(collection(db, this.subjectsCollection));
    ...
    async clearSubjectMarks(subjectId: string, studentIds: string[]): Promise<void> {
        ...
        try {
            ...
            const updatePromises = studentIds.map(async (studentId) => {
                const studentDoc = await getDoc(doc(db, this.studentsCollection, studentId));
                if (!studentDoc.exists()) {
